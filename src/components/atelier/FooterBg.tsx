...
function Mark({ x, y, size }: { x: number; y: number; size: number }) {
    return (
        <svg x={x} y={y} width={size} height={size} viewBox="260 300 560 480" overflow="visible">
            <path d={MARK_PATH} fill="#222222" fillRule="evenodd" />
            {/* right shadow face */}
            <path d={MARK_PATH} fill="none" stroke="#0a0a0a" strokeWidth="18" strokeLinejoin="round" />
        </svg>
    );
}
...
export function FooterBg() {
    ...
    return (
        ...
        >
            <defs>
                <filter id="mark-depth" x="-10%" y="-10%" width="130%" height="130%">
                    <feDropShadow dx="5" dy="6" stdDeviation="3" floodColor="#000000" floodOpacity="0.9" />
                </filter>
                {/* staggered tile: 2 cells wide × 2 cells tall, marks offset by (S, S) diagonally */}
                <pattern id="footer-mark-tile" x="0" y="0" width={S * 2} height={S * 2} patternUnits="userSpaceOnUse">
                    <rect width={S * 2} height={S * 2} fill="#080808" />
                    <g filter="url(#mark-depth)">
                        <Mark x={pad} y={pad} size={M} />
                        <Mark x={pad + S} y={pad + S} size={M} />
                    </g>
                </pattern>
            </defs>
            <rect width="100%" height="100%" fill="url(#footer-mark-tile)" />
        </svg>
    );
}
